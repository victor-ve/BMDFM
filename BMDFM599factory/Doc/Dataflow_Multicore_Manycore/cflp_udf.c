...
void dftest_write_data_to_cluster(const ULO *dat_ptr,
  ...
  if(noterror()){
    if(0==DataClusterInMemory_ptr)
      rise_error_info(ECODE_RT__DFTEST_OUT_OF_MEMORY,
        "dftest_write_data_to_cluster(): memory allocation failure");
    else{
...
void dftest_read_data_to_cluster(const ULO *dat_ptr,
  ...
  if(noterror()){
    if(0==DataClusterInMemory_ptr)
      rise_error_info(ECODE_RT__DFTEST_OUT_OF_MEMORY,
        "dftest_read_data_to_cluster(): memory allocation failure");
    else
      if(((-1==*file_descr_p||!cmp(InputTestFileName,*FileName_p)))&&
...
void dftest_process_data_cluster(const ULO *dat_ptr,
  ...
  if(noterror()){
    if(0==DataClusterInMemory_ptr)
      rise_error_info(ECODE_RT__DFTEST_OUT_OF_MEMORY,
        "dftest_process_data_cluster(): memory allocation failure");
    else{
